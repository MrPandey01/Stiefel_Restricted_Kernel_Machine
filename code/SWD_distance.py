...
def sliced_wasserstein_distance(generated_images, x, sample_nr=26000):
    generated_images = tf.convert_to_tensor(generated_images[:sample_nr])
    x = tf.convert_to_tensor(x[:sample_nr])
    print('X: {}, Gen: {}'.format(x.shape, generated_images.shape))
    distance = _sliced_wasserstein_distance(x, generated_images)
    distances_fake = []
    for d in distance:
        distances_fake.append(d[1])
    return np.asarray(distances_fake)
